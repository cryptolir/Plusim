...
- **Installment (תשלומים/קרדיט) transactions** — **2026-08 ✅** credit-card statements print the ORIGINAL DEAL date on installment rows, not the date they are charged, so a single statement scattered its charges across months that were not in the report (measured: 5 rows, ₪1,825.03, 4 phantom months). `parse_leumi_pdf.py` now re-dates each installment to the charge date of the total block that closes it — bounded to that block, so a missing date can never be taken from the next row — keeping the deal date in the note. A block with no printed date falls back to the statement's own header date; a row that gets neither keeps its deal date and is surfaced as a non-blocking note for the admin to correct — a missing charge date never blocks the report. Rows carry an N-of-M badge through the admin tables, the online report and `/report`, and an admin can correct any transaction's date (the write is conditional on the job not being mid-run, which also closed the same pre-existing race on category assignment). See [docs/plans/installment-transactions.md](./docs/plans/installment-transactions.md).
