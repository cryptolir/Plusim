...
The admin meeting-transcripts feature: browse a shared Drive folder, assign a subfolder per user, summarize transcripts with the `onlyclaw` agent (saved back to Drive), and a home **"Past meeting"** prompt that resumes chat grounded in the latest summary.
...
- **Summarize trigger** — `POST /admin/api/drive/summarize` reads the transcript (Google Doc → export `text/plain`; `text/*` → `alt=media`; other types rejected), truncates past ~24k chars (logged), and generates the summary with a **direct OpenAI call** (`generateText`, `@ai-sdk/openai`) — not the agent runtime — for speed and reliability at any size. The prompt embeds the **summary method** (next bullet) as a `=== METHOD ===` block and asks the model to lead with `TITLE:` / `DATE:` lines (parsed for the filename; else the source filename + Drive `createdTime` are used). Requires `OPENAI_API_KEY`.
- **Summary method (admin-editable)** — the method/structure applied to the transcript is stored in `AppSetting.summary_instructions` (plaintext) and edited on **`/admin/settings`** (the `/admin/drive` page links there). `getSummaryInstructions()` (`src/lib/summaryInstructions.ts`) returns the stored value, or the built-in **financial-meeting method** (`DEFAULT_SUMMARY_INSTRUCTIONS`) when blank.
- **"Past meeting"** — `buildLinkedFolderContext(userId)` (`src/lib/pastMeeting.ts`) fetches the latest summary from the user's assigned folder and injects it as the invisible first-turn preamble in `/api/chat` (for the `past_meeting` pin and plain chats). The transcript never appears in the chat UI or the stored `Message.content`.
